...
//    stockTaskService.onRunTask(new TaskParams(intent.getStringExtra("tag"), args));
        ...
        if (stockTaskService.onRunTask(new TaskParams(intent.getStringExtra(getResources().getString(R.string.string_tag)), args)) == GcmNetworkManager.RESULT_FAILURE) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(getApplicationContext(), getString(R.string.entity_error), Toast.LENGTH_LONG).show();
                }
            });
        } else {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(getApplicationContext(),getString(R.string.stock_found), Toast.LENGTH_LONG).show();
                }
            });
        }
    }
